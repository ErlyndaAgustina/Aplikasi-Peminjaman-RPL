...
class _ManajemenKategoriPageState extends State<ManajemenKategoriPage> {
  // Pindahkan variabel ke level class agar state tersimpan
  String _searchQuery = "";
  ...
  void _showKategoriForm({KategoriModel? kategori, int? index}) {
    ...
    final nameController = TextEditingController(
      text: isEdit ? kategori.nama : "",
    );
    final descController = TextEditingController(
      text: isEdit ? kategori.deskripsi : "",
    );
    ...
    showDialog(
      ...
      builder: (context) => KategoriFormDialog(
        ...
        onSave: () {
          setState(() {
            if (isEdit) {
              kategoriList[index!] = KategoriModel(
                nama: nameController.text,
                deskripsi: descController.text,
                totalAlat: kategori.totalAlat,
              );
            } else {
              kategoriList.add(
                KategoriModel(
  ...
  void _showDeleteConfirmation(int index) {
    showDialog(
      ...
      builder: (context) => DeleteConfirmDialog(
        onDelete: () {
          setState(() => kategoriList.removeAt(index));
          Navigator.pop(context);
        },
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final filteredList = kategoriList.where((kat) {
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final originalIndex = kategoriList.indexOf(kategori);
                  return KategoriCard(
                    kategori: kategori,
                    onEdit: () => _showKategoriForm(
                      kategori: kategori,
                      index: originalIndex,
                    ),
                    onDelete: () => _showDeleteConfirmation(originalIndex),
                  );
                },
              ),
            ),
