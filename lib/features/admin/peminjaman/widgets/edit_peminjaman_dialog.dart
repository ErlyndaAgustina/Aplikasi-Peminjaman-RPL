...
class EditPeminjamanDialog extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Flexible( // Pakai Flexible supaya Column di dalamnya bisa scroll
                        ...
                        return Row(
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start, 
                                children: [_label('Tanggal Pinjam'), _dateField(data.tanggal)]
                              )
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start, 
                                children: [_label('Batas Kembali'), _dateField(data.tanggal)]
                              )
                            ),
                          ],
                        );
  ...
  Widget _label(String t) => Padding(
    ...
    child: Text(t, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF312F34)))
  );
  ...
  Widget _field(String val, {bool enabled = true}) => TextFormField(
    ...
    decoration: InputDecoration(
      ...
      disabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFCDEEE2))),
    ),
  );
  ...
  Widget _dateField(String val) => Container(
    ...
    child: Row(children: [
      Expanded(child: Text(val, style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis)), 
  ...
  Widget _btn(String t, BuildContext ctx, {bool isOutlined = false}) => SizedBox(
    ...
    child: ElevatedButton(
      ...
      child: Text(t, style: TextStyle(color: isOutlined ? const Color(0xFF3E9F7F) : Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
    ),
  );
}